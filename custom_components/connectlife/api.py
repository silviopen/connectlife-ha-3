...
class ConnectLifeApi:
    ...
    def _normalize_appliance_payloads(self, appliances: Any) -> Any:
        ...
        if not isinstance(appliances, list) or not self.appliances:
            return self._drop_incomplete_appliance_payloads(appliances)

        previous_status_lists = {
            appliance.device_id: appliance.status_list
            for appliance in self.appliances
            if getattr(appliance, "device_id", None) is not None
            and getattr(appliance, "status_list", None) is not None
        }
        if not previous_status_lists:
            return appliances

        merged_appliances: list[Any] = []
        for appliance in appliances:
            if not isinstance(appliance, dict):
                merged_appliances.append(appliance)
                continue

            device_id = appliance.get("deviceId")
            if "statusList" not in appliance and device_id in previous_status_lists:
                merged_appliances.append(
                    {
                        **appliance,
                        "statusList": previous_status_lists[device_id],
                    }
                )
                continue
            merged_appliances.append(appliance)
        return self._drop_incomplete_appliance_payloads(merged_appliances)

    def _drop_incomplete_appliance_payloads(self, appliances: Any) -> Any:
        """Drop appliance payloads that are missing a required status list."""
        if not isinstance(appliances, list):
            return appliances

        normalized_appliances: list[Any] = []
        for appliance in appliances:
            if (
                isinstance(appliance, dict)
                and "deviceId" in appliance
                and "statusList" not in appliance
            ):
                _LOGGER.warning(
                    "Dropping ConnectLife appliance payload for %s because it is missing statusList",
                    appliance["deviceId"],
                )
                continue
            normalized_appliances.append(appliance)
        return normalized_appliances
    ...
    async def update_appliance(self, puid: str, properties: dict[str, str]) -> None:
        ...
        await self._update_bapi_appliance(data, retry_on_reauth=True)
    ...
    async def _update_bapi_appliance(
        ...
        *,
        ...
        try:
            async with self._client_session() as session:
                async with session.post(
                    ...
                    if response.status != 200:
                        ...
                        if retry_on_reauth and response.status in TRANSIENT_STATUSES:
                            _LOGGER.warning(
                                "ConnectLife update via bapi failed with status=%s, retrying after re-authentication",
                                response.status,
                            )
                            await self.login()
                            return await self._update_bapi_appliance(
                                data,
            ...
            if retry_on_reauth:
                _LOGGER.warning(
                    "ConnectLife update via bapi failed with transport error, retrying after re-authentication: %s",
                    err,
                )
                await self.login()
                return await self._update_bapi_appliance(data, retry_on_reauth=False)
            raise LifeConnectError(
                f"Unexpected transport error from bapi: {err}",
                endpoint=self.appliances_url,
            ) from err
    ...
    async def _request_gateway_json(
        ...
        *,
        ...
        if retry_on_randstr and error_code == GATEWAY_RANDSTR_CHECK_FAILED:
            _LOGGER.warning(
                "HijuConn gateway rejected randStr, retrying once with a fresh signature"
            )
            return await self._request_gateway_json(
                url,
                payload=payload,
        ...
        error_type = (
            ...
            if error_code == GATEWAY_INVALID_ACCESS_TOKEN
    ...
    async def _json(
        ...
        *,
        ...
        try:
            ...
            raise error_type(
                f"Unexpected non-JSON response from {endpoint}",
